...
export default function StanokchiMachineDetail() {
    ...
    /* ── PRODUCE tarixi (product-stock-transactions?action=PRODUCE) ── */
    const [historyPage, setHistoryPage] = useState(0);
    const { data: historyResult, isLoading: historyLoading, refetch: refetchHistory } =
        useGetStockTransactionsQuery(
            { action: 'PRODUCE', page: historyPage, size: HISTORY_PAGE_SIZE },
            { skip: !id }
        );
    const historyItems = historyResult?.data ?? historyResult?.items ?? [];
                                ...
                                <table className="w-full text-sm border-collapse">
                                    <thead>
                                        <tr style={{
                                            ...
                                            {['№', 'Mahsulot', 'Miqdor (dona)', 'Sana'].map((h) => (
