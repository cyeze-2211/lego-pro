...
export default function Machine() {
    ...
    const { data: machineResult, isLoading, error } = useGetMachinesQuery({
        name: query || undefined, page, size: PAGE_SIZE,
    });
    ...
    const submitSearch = (e) => { e.preventDefault(); setPage(0); setQuery(search.trim()); };
    const clearSearch  = () => { setSearch(''); setPage(0); setQuery(''); };
    ...
    const renderStatus = (machine) => {
        ...
        if (isDefect) return (
            <Badge variant="subtle" borderRadius="full" px={3} py={1}
                ...
                color={isDark ? 'red.300' : '#991B1B'}
                whiteSpace="nowrap"
                title={`Boshlangan: ${formatStartedAt(machine.currentRun?.startedAt)}`}>
                ⚠ Brak: {machine.currentRun?.productName}
            </Badge>
        );
    ...
    return (
        <Box my={2} bg={pageBg} color={textColor} minH="100%">
            ...
            >
                <Box position="relative" flex="1" w="100%">
                    ...
                    {search && (
                        <Button
                            ...
                            color={subtitleColor} minW="32px" h="32px" p={0} borderRadius="full"
                ...
                <Button
                    type="submit" bg={accentColor} color="black" borderRadius="xl"
                    minH="32px" px={6} flexShrink={0}
                            ...
                            <Table.Header bg={tableHeaderBg}>
                                <Table.Row bg={tableHeaderBg}>
                                    <Table.ColumnHeader textAlign="center" w="60px"
                                        bg={tableHeaderBg} borderWidth="0.5px" borderColor={tableBorder} color={subtitleColor}>
                                        №
                                    </Table.ColumnHeader>
                            ...
                            <Table.Body bg={tableBg}>
                                {machines.map((machine, index) => (
                                    ...
                                    >
                                        ...
                                        <Table.Cell borderWidth="0.5px" borderColor={tableBorder} color={subtitleColor} maxW="320px">
                                            {machine.summary || '—'}
                                        </Table.Cell>
