...
import { useDeleteCashboxMutation } from '../../../../store/services/cashbox.api';
import { Alert } from '../../../Other/UI/Alert/Alert';
import { useAppTheme } from '../../../../theme/tokens';

export default function Delete({ cashbox }) {
    const { open, onOpen, onClose } = useDisclosure();
    const [deleteCashbox, { isLoading }] = useDeleteCashboxMutation();
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            Alert('Kassa o‘chirildi', 'success');
            onClose();
        } catch (error) {
            Alert(error?.data?.message || 'Kassani o‘chirishda xatolik', 'error');
        }
    };
                                    ...
                                    <Box p={4} borderRadius="xl" bg={isDark ? 'rgba(239, 68, 68, 0.08)' : 'red.50'} borderWidth="1px" borderColor={isDark ? 'rgba(239, 68, 68, 0.2)' : 'red.200'}>
                                        <HStack gap={3} align="start">
                                            ...
                                            <Text fontSize="sm" color={isDark ? 'red.300' : 'red.700'}>
                                                Siz <strong>“{cashbox.name}”</strong> kassasini o‘chirmoqchisiz.
                                            </Text>
                                        </HStack>
                                    </Box>
                                    <Text fontSize="sm" color={subtitleColor} pl={1}>
                                        Kassa ro‘yxatdan olib tashlanadi. Agar kassaga bog‘liq faol xarajatlar bo‘lsa, avval ularni o‘chiring.
                                    </Text>
                                </VStack>
                            </Dialog.Body>
