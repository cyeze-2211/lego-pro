...
export default function Delete({ product }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            Alert('Mahsulot o‘chirildi', 'success');
            onClose();
        } catch (error) {
            Alert(error?.data?.message || 'Mahsulotni o‘chirishda xatolik', 'error');
        }
    };
                                    ...
                                    <Box p={4} borderRadius="xl" bg={isDark ? 'rgba(239, 68, 68, 0.08)' : 'red.50'} borderWidth="1px" borderColor={isDark ? 'rgba(239, 68, 68, 0.2)' : 'red.200'}>
                                        <HStack gap={3} align="start">
                                            ...
                                            <Text fontSize="sm" color={isDark ? 'red.300' : 'red.700'}>
                                                Siz <strong>“{product.name}”</strong> mahsulotini o‘chirmoqchisiz. Bu amalni qaytarib bo‘lmaydi.
                                            </Text>
                                        </HStack>
                                    </Box>
                                    <Text fontSize="sm" color={subtitleColor} pl={1}>
                                        Mahsulot ro‘yxatdan olib tashlanadi, ammo uning tarixi saqlanib qoladi.
                                    </Text>
                                </VStack>
                            </Dialog.Body>
