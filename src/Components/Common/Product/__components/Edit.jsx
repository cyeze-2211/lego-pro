...
import { LuDollarSign, LuPen, LuTag } from 'react-icons/lu';
...
export default function Edit({ product }) {
    ...
    const handleSubmit = async () => {
        ...
        if (!name.trim() || !price || Number(price) <= 0) {
            Alert('Mahsulot nomi va narxi majburiy', 'error');
            return;
        }
        try {
            await updateProduct({ id: product.id, data: { name: name.trim(), price: Number(price) } }).unwrap();
                        ...
                        <Dialog.Content bg={cardBg} borderColor={modalBorder} borderWidth="1px" borderRadius="2xl" boxShadow="2xl" overflow="hidden" maxW="520px" w="calc(100% - 32px)">
                            ...
                            <Dialog.Body py={6}>
                                <VStack gap={6} align="stretch">
                                    <Field.Root required>
                                        <Field.Label color={textColor} fontWeight="medium"><HStack gap={2}><LuTag size={16} /><span>Mahsulot nomi</span></HStack><Field.RequiredIndicator /></Field.Label>
                                        <FormControl value={name} onChange={(event) => setName(event.target.value)} placeholder="Masalan, Shakar" autoFocus minH="52px" />
                                    </Field.Root>
                                    <Field.Root required>
                                        ...
                                        <FormattedNumberInput value={price} onChange={setPrice} placeholder="25 000" min="0.01" step="0.01" minH="52px" />
                                    </Field.Root>
                                </VStack>
                            </Dialog.Body>
