...
import { LuCalendar, LuMonitorCog, LuShieldCheck, LuUserRound } from 'react-icons/lu';
...
import { AuditSection } from '../DeviceDetail';
...
export default function UserDetail() {
    ...
    return (
        <EntityDetail
            title={user?.username || 'Foydalanuvchi'}
            icon={LuUserRound}
            backTo="/users"
            backLabel="Foydalanuvchilar"
            loading={isLoading}
            error={isError || !user}
        >
            {() => (
                <>
                    <DetailSection title="Foydalanuvchi ma'lumotlari" icon={LuUserRound}>
                        ...
                        <DetailRow label="Rol" value={
                            <Badge borderRadius="full" px={3} py={1}
                                bg="rgba(250,204,21,.14)" color="yellow.700">
                                {user.roleName}
                            </Badge>
                        } />
                        <DetailRow
                            label="Oxirgi kirish"
                            value={user.lastLoginAt ? formatDetailDate(user.lastLoginAt) : 'Kirilmagan'}
                        />
                    </DetailSection>
