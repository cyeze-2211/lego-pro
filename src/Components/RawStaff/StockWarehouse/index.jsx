...
import {
    LuSearch, LuWarehouse, LuChevronLeft, LuChevronRight,
    LuPackage, LuX, LuFlame,
} from 'react-icons/lu';
                ...
                <div className="relative flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className={`text-xs mt-0.5 ${muted}`}>Omborlardagi xom ashyo qoldiqlarini kuzating</p>
                        </div>
                    </div>
                    <span className="inline-flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-bold bg-amber-400/10 text-amber-500 border-amber-400/20">
                        {pagination.totalElements ?? 0} qator
                    </span>
                </div>
            </div>
                                        ...
                                        <tr key={`${s.rawMaterialId}-${s.warehouseId}`} className={`transition-colors ${rowHov}`}>
                                            ...
                                            <td className="px-5 py-3.5 text-right">
                                                <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${
                                                    s.quantity > 0 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'
                                                }`}>
                                                    {typeof s.quantity === 'number' ? s.quantity.toLocaleString('uz-UZ', { maximumFractionDigits: 3 }) : s.quantity} {s.unit || unit}
                                                </span>
                                            </td>
                                        </tr>
                                    ))}
