                ...
                <div className={`rounded-2xl border shadow-md ${panel}`}>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className={`text-left text-xs font-semibold uppercase tracking-wide ${muted} ${isDark ? 'bg-[#0f172a]/30' : 'bg-[#f8fafc]/80'}`}>
                                        ...
                                        <th className="px-5 py-3 w-36 text-right">Narx</th>
                                    ...
                                    {items.map((item, idx) => (
                                        <tr key={item.productId} className={`transition-colors ${rowBg}`}>
                                            ...
                                            <td className={`px-5 py-3 text-right font-bold ${head}`}>
                                                {(item.quantity * (item.unitPrice ?? 0)).toLocaleString('uz-UZ')} so&apos;m
                                            </td>
                    ...
                    <div className={`flex flex-wrap items-center justify-between gap-3 border-t px-5 py-4 ${line}`}>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                            ...
                            <span className={`text-sm ${muted}`}>
                                Summa: <span className={`font-bold ${head}`}>{totalAmount.toLocaleString('uz-UZ')} so&apos;m</span>
                            </span>
                            {selectedWarehouse && (
