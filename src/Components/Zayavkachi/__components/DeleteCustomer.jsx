...
import { useDeleteCustomerMutation } from '../../../store/services/customer.api';
import { Alert } from '../../Other/UI/Alert/Alert';
import { useAppTheme } from '../../../theme/tokens';

export default function DeleteCustomer({ customer, disabled = false, onDeleted }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            Alert('Mijoz o‘chirildi', 'success');
            onClose();
            if (onDeleted) onDeleted();
        } catch (error) {
            Alert(error?.data?.message || 'Mijozni o‘chirishda xatolik', 'error');
        }
    };
                                    ...
                                    <Box p={4} borderRadius="xl" bg={isDark ? 'rgba(239, 68, 68, 0.08)' : 'red.50'} borderWidth="1px" borderColor={isDark ? 'rgba(239, 68, 68, 0.2)' : 'red.200'}>
                                        <HStack gap={3} align="start">
                                            ...
                                            <Text fontSize="sm" color={isDark ? 'red.300' : 'red.700'}>
                                                Siz <strong>“{customer.name}”</strong> mijozini o‘chirmoqchisiz. Bu amalni qaytarib bo‘lmaydi.
                                            </Text>
                                        </HStack>
                                    </Box>
                                    <Text fontSize="sm" color={subtitleColor} pl={1}>
                                        Mijoz ro‘yxatdan olib tashlanadi.
                                    </Text>
                                </VStack>
                            </Dialog.Body>
