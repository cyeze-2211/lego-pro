...
import { useDeleteSalesOrderMutation } from '../../../store/services/salesOrder.api';
import { Alert } from '../../Other/UI/Alert/Alert';
import { useAppTheme } from '../../../theme/tokens';

export default function DeleteOrder({ order, disabled = false, onDeleted }) {
    const { open, onOpen, onClose } = useDisclosure();
    const [deleteOrder, { isLoading }] = useDeleteSalesOrderMutation();
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            Alert('Buyurtma o‘chirildi', 'success');
            onClose();
            if (onDeleted) onDeleted();
        } catch (error) {
            Alert(error?.data?.message || 'Buyurtmani o‘chirishda xatolik', 'error');
        }
    };
    ...
    return (
        <>
            <Button
                ...
                title={disabled ? 'Faqat “Kutilmoqda” holatidagi buyurtma o‘chiriladi' : 'O‘chirish'}
                                    ...
                                    <Box p={4} borderRadius="xl" bg={isDark ? 'rgba(239, 68, 68, 0.08)' : 'red.50'} borderWidth="1px" borderColor={isDark ? 'rgba(239, 68, 68, 0.2)' : 'red.200'}>
                                        <HStack gap={3} align="start">
                                            ...
                                            <Text fontSize="sm" color={isDark ? 'red.300' : 'red.700'}>
                                                Siz <strong>“{order.customerName}”</strong> uchun yaratilgan buyurtmani o‘chirmoqchisiz. Bu amalni qaytarib bo‘lmaydi.
                                            </Text>
                                        </HStack>
                                    </Box>
