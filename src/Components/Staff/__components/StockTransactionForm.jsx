...
export default function StockTransactionForm({ action }) {
    ...
    const submitBtn = isIn
        ...
        : 'bg-rose-500 hover:bg-rose-600 active:bg-rose-700 text-white shadow-rose-500/20';
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className={`text-xs mt-0.5 ${muted}`}>
                                {isIn ? 'Omborga qabul qilinadigan mahsulotlar' : 'Ombordan chiqariladigan mahsulotlar'}
                            </p>
                        </div>
                    </div>
                    {items.length > 0 && (
                        <span className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-bold ${badge}`}>
                            <LuPackage size={12} /> {items.length} mahsulot · {totalQty} dona
                        </span>
                    )}
                </div>
            </div>
                ...
                <div className={`rounded-2xl border p-4 shadow-md ${panel}`}>
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                        ...
                        <div className="flex-1">
                            <label className={`mb-1.5 flex items-center gap-1.5 text-xs font-semibold ${muted}`}>
                                <LuSearch size={12} /> Mahsulot qidirish
                            </label>
                            <div className="relative">
                                <LuSearch className={`pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 ${muted}`} />
                                <input
                                    type="text"
                                    placeholder={warehouseId ? 'Mahsulot nomini yozing...' : 'Avval ombor tanlang'}                                    value={search}
                                ...
                                {search && (
                                    <button
                                        type="button"
                                        onClick={() => setSearch('')}
                                        className={`absolute right-3 top-1/2 -translate-y-1/2 transition-colors ${muted} hover:text-slate-700 dark:hover:text-white`}
                                    >
                                        <LuX size={14} />
                                    </button>
                                )}
                    ...
                    {/* Search dropdown — ombor tanlansa doim ko'rsatiladi */}
                    {warehouseId && (
                        <div className={`mt-2 overflow-hidden rounded-xl border shadow-xl ${isDark ? 'border-slate-700 bg-[#0f172a]' : 'border-slate-200 bg-white'}`}>
                            {isFetching ? (
                                ...
                                <div className={`py-5 text-center text-sm ${muted}`}>Mahsulot topilmadi</div>
                            ) : (
                ...
                <div className={`rounded-2xl border shadow-md ${panel}`}>
                    ...
                    {items.length === 0 ? (
                        <div className={`flex flex-col items-center gap-2 py-12 ${muted}`}>
                            <LuPackage size={32} strokeWidth={1.5} />
                            <p className="text-sm font-medium">Mahsulot tanlanmagan</p>
                            <p className="text-xs">Yuqoridagi qidiruvdan mahsulot qo&apos;shing</p>
                        </div>
                    ) : (
                        <>
                            {/* Desktop */}
                                    ...
                                    <tbody className={`divide-y ${divider}`}>
                                        {items.map((item) => (
                                            <tr key={item.productId} className={`transition-colors ${rowBg}`}>
                                                <td className={`px-5 py-3.5 font-semibold ${head}`}>{item.productName}</td>
                                                <td className={`px-5 py-3.5 font-mono text-xs ${muted}`}>
                                                    <span className="flex items-center gap-1"><LuBarcode size={12} />{item.productBarcode}</span>
                                                </td>
                                                <td className="px-5 py-3.5">
                                                    ...
                                                    <div className={`inline-flex items-center rounded-xl border overflow-hidden ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                                                        <button
                                                            type="button"
                                                            onClick={() => stepQty(item.productId, -1)}
                                    ...
                                    <div key={item.productId} className={`px-4 py-3.5 transition-colors ${rowBg}`}>
                                        <div className="flex items-start justify-between gap-2 mb-3">
                                            <div className="min-w-0">
                                                <p className={`font-semibold truncate text-sm ${head}`}>{item.productName}</p>
                                                <p className={`text-xs font-mono ${muted}`}>{item.productBarcode}</p>
                                            </div>
                                            <button
                                                type="button"
                                                onClick={() => removeItem(item.productId)}
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className={`text-xs font-medium ${muted}`}>Miqdor:</span>
                                            <div className={`inline-flex items-center rounded-xl border overflow-hidden ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                                                ...
                                                <input type="number" min={1} value={item.quantity}
                                                    onChange={(e) => updateQty(item.productId, e.target.value)}
                                                    className={`h-8 w-12 border-x text-center text-sm font-bold outline-none ${isDark ? 'border-slate-700 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-900'}`}
                                                />
                                                <button type="button" onClick={() => stepQty(item.productId, 1)}
                                                    className={`flex h-8 w-8 items-center justify-center ${isDark ? 'bg-slate-800 hover:bg-slate-700' : 'bg-slate-50 hover:bg-slate-100'}`}>
                                                    <LuPlus size={12} />
                                            ...
                                            <span className={`text-xs ${muted}`}>dona</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className={`flex flex-wrap items-center justify-between gap-3 border-t px-5 py-3.5 ${isDark ? 'border-slate-700/60' : 'border-slate-100'}`}>
                                <div className="flex items-center gap-4">
                                    <span className={`text-sm ${muted}`}>
                                        Jami: <span className={`font-bold ${head}`}>{items.length} mahsulot</span>
                                    </span>
                                    <span className={`text-sm ${muted}`}>
