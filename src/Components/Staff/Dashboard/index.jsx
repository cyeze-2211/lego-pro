...
import {
    ...
    LuLogIn, LuLogOut, LuWarehouse, LuTrendingUp, LuTrendingDown,
} from 'react-icons/lu';
import { useAppTheme } from '../../../theme/tokens';
import { useGetProductStocksQuery, useGetStockTransactionsQuery } from '../../../store/services/productStock.api';
...
export default function StaffDashboard() {
    ...
    const { data: stocksData } = useGetProductStocksQuery({ size: 5 });
    const { data: inData  }    = useGetStockTransactionsQuery({ action: 'IN',  size: 5 });
    const { data: outData }    = useGetStockTransactionsQuery({ action: 'OUT', size: 5 });
    ...
    return (
        <div className="flex w-full flex-col gap-4 py-2">
            ...
            <div className={`relative overflow-hidden rounded-2xl border px-5 py-4 shadow-md ${panel}`}>
                ...
                <div className="relative flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p className={`mt-0.5 text-xs ${muted}`}>Kirim, chiqim va qoldiqlarni nazorat qiling.</p>
                    </div>
            ...
            <div className="grid grid-cols-3 gap-3">
                {[
                    { label: 'Mahsulot turlari', value: totalStock, icon: LuBoxes,        cls: 'bg-amber-400/10 text-amber-500'   },
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                ...
                <div className={`rounded-2xl border shadow-md ${panel}`}>
                    <div className={`flex items-center justify-between border-b px-5 py-3.5 ${isDark ? 'border-slate-700/60' : 'border-slate-100'}`}>
                        ...
                        <Link to="/staff/history" className={`flex items-center gap-1 text-xs font-semibold text-amber-500 hover:text-amber-600`}>
                        ...
                        {recentIn.length === 0 ? (
                            ...
                            <div key={item.id} className={`flex items-center justify-between px-5 py-3 transition-colors ${rowBg}`}>
                                <div className="min-w-0">
                                    <p className={`truncate text-sm font-semibold ${head}`}>{item.productName}</p>
            ...
            <div className={`rounded-2xl border shadow-md ${panel}`}>
                <div className={`flex items-center justify-between border-b px-5 py-3.5 ${isDark ? 'border-slate-700/60' : 'border-slate-100'}`}>
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <p className={`text-sm font-bold ${head}`}>Joriy qoldiqlar</p>
                            <p className={`text-xs ${muted}`}>Omborlardagi mahsulotlar</p>
                        </div>
                    </div>
                        ...
                        <tbody className={`divide-y ${divider}`}>
                            {stocks.length === 0 ? (
                                ...
                                <tr key={`${s.productId}-${s.warehouseId}`} className={`transition-colors ${rowBg}`}>
                                    <td className={`px-5 py-3 font-semibold ${head}`}>{s.productName}</td>
